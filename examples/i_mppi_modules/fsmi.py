...
class FSMIConfig:
    ...
    goal_pos: jax.Array = field(default_factory=lambda: jnp.array([9.0, 5.0, -2.0]))
...
class FSMITrajectoryGenerator:
    ...
    def get_target(self, info_levels: jax.Array) -> tuple[jax.Array, str]:
        """Determine target based on information levels (Simple State Machine)."""
        ...
        if info_levels[0] > self.config.info_threshold:
            target_pos = jnp.array([self.info_zones[0,0], self.info_zones[0,1], -2.0])
            target_mode = "INFO 1"
        elif info_levels[1] > self.config.info_threshold:
            target_pos = jnp.array([self.info_zones[1,0], self.info_zones[1,1], -2.0])
            target_mode = "INFO 2"
