...
def compute_full_map_fsmi(fsmi_module, grid_array, flat_positions, yaws,
                          Nx, Ny, map_origin, map_resolution):
    ...
    gx_idx = ((flat_positions[:, 0] - map_origin[0]) / map_resolution).astype(jnp.int32)
    gy_idx = ((flat_positions[:, 1] - map_origin[1]) / map_resolution).astype(jnp.int32)
...
def plot_field_panel(ax, fig, grid_array, map_resolution, field_np, field_origin,
                     field_res, title, annotate_points=None, trajectory=None):
    """Plot grid + FSMI field heatmap on a single axis."""
    H, W = grid_array.shape
    grid_extent = [0, W * map_resolution, 0, H * map_resolution]
    ax.imshow(np.array(grid_array), origin="lower", extent=grid_extent,
              cmap="gray_r", vmin=0, vmax=1, alpha=0.5)
    ...
    im = ax.imshow(field_np.T, origin="lower", extent=field_extent,
                   cmap="YlOrBr_r", alpha=0.7, interpolation="bilinear",
                   vmin=0)
    fig.colorbar(im, ax=ax, shrink=0.7, label="FSMI gain")

    # Mark start and goal
    ax.plot(START_X, START_Y, "go", markersize=10, label="Start", zorder=5)
    ax.plot(float(GOAL_POS[0]), float(GOAL_POS[1]), "r*", markersize=14,
            label="Goal", zorder=5)
    ...
    for i in range(len(INFO_ZONES)):
        ...
        ax.add_patch(rect)
        ax.text(cx, cy + h / 2 + 0.3, f"Zone {i+1}", ha="center",
    ...
    if annotate_points is not None:
        for px, py, val in annotate_points:
            ...
            ax.annotate(f"{val:.2f}", (px, py), textcoords="offset points",
                        xytext=(5, 5), fontsize=7, color="black",
                        bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.7))

    # Gradient trajectory
    if trajectory is not None:
        ax.plot(trajectory[:, 0], trajectory[:, 1], "c-o", markersize=3,
                linewidth=2, label="Gradient traj", zorder=6)
...
def main():
    ...
    _compute_field = jax.jit(
        lambda g: compute_full_map_fsmi(
            fsmi_module, g, flat_positions, yaws, Nx, Ny,
    ...
    print(f"  Field range: [{float(field.min()):.4f}, {float(field.max()):.4f}]")
    ...
    for px, py in sample_points_xy:
        val = float(interp2d(field, field_origin, FIELD_RES, jnp.array([px, py])))
    ...
    print(f"  Field range: [{float(field_depleted.min()):.4f}, {float(field_depleted.max()):.4f}]")

    # Annotated points on depleted field
    annotate_pts_depleted = []
    for px, py in sample_points_xy:
        val = float(interp2d(field_depleted, field_origin, FIELD_RES, jnp.array([px, py])))
    ...
    ref_traj = field_gradient_trajectory(
        field, field_origin, FIELD_RES,
        uav_xy, HORIZON, REF_SPEED, DT, ALTITUDE,
    )
    ref_traj_np = np.array(ref_traj)
    print(f"  Trajectory: ({ref_traj_np[0,0]:.2f}, {ref_traj_np[0,1]:.2f}) -> "
    ...
    plot_field_panel(
        axes[0], fig, grid_np, map_resolution, field_np, field_origin,
        FIELD_RES, "FSMI Field (Undepleted) + Gradient Trajectory",
        annotate_points=annotate_pts, trajectory=ref_traj_np,
    )

    # Panel 2: Full-map FSMI (zone 1 depleted) with sample points
    plot_field_panel(
        axes[1], fig, depleted_grid_np, map_resolution, field_depleted_np,
        field_origin, FIELD_RES, "FSMI Field (Zone 1 Depleted)",
    ...
    ax.imshow(grid_np, origin="lower", extent=grid_extent,
              cmap="gray_r", vmin=0, vmax=1, alpha=0.5)
    Nx, Ny = diff_np.shape
    fx0, fy0 = float(field_origin[0]), float(field_origin[1])
    diff_extent = [fx0, fx0 + Nx * FIELD_RES, fy0, fy0 + Ny * FIELD_RES]
    im = ax.imshow(diff_np.T, origin="lower", extent=diff_extent,
                   cmap="RdYlGn", alpha=0.7, interpolation="bilinear")
    fig.colorbar(im, ax=ax, shrink=0.7, label="FSMI drop (before - after)")
    ax.plot(START_X, START_Y, "go", markersize=10, zorder=5)
    ax.plot(float(GOAL_POS[0]), float(GOAL_POS[1]), "r*", markersize=14, zorder=5)
    for i in range(len(INFO_ZONES)):
        cx, cy = float(INFO_ZONES[i, 0]), float(INFO_ZONES[i, 1])
        w, h = float(INFO_ZONES[i, 2]), float(INFO_ZONES[i, 3])
        rect = plt.Rectangle(  # pyright: ignore[reportPrivateImportUsage]
    ...
    save_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "..", "..", "docs", "_media", "i_mppi", "debug_fsmi_field.png"
    )
