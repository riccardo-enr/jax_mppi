...
The autotuning components require additional dependencies.

```bash
# Install core autotuning (cma, evosax)
pip install jax-mppi[autotuning]

# Install extra autotuning tools (Ray Tune, Hyperopt, Ribs)
...
def evaluate():
    ...
    return autotune.EvaluationResult(mean_cost=cost, params={})
...
:::{.callout-note}
## Requirements
This requires `ray[tune]`, `hyperopt`, and `bayesian-optimization`.
:::
