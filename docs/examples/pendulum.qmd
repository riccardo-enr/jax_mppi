...
The pendulum swing-up is a classic control problem where the controller must use limited torque to pump energy into the system, swing the pendulum upright, and then stabilize it.
...
def pendulum_cost(state: jax.Array, action: jax.Array) -> jax.Array:
    """Calculate the cost for a state and action.
    """
    theta, theta_dot = state[0], state[1]
    torque = action[0]
