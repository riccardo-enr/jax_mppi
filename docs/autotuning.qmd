...
# 1. Setup MPPI
# Assuming `dynamics` and `cost_fn` are previously defined
...
# Setup MPPI and holder (similar to Quick Start)
# ...
...
# Setup parameters to tune and evaluate function
# ...
...
|-----------|------|-------------|
| `LambdaParameter` | `TunableParameter` | MPPI temperature ($\lambda$). |
| `NoiseSigmaParameter` | `TunableParameter` | Exploration noise covariance diagonal. |
| `MuParameter` | `TunableParameter` | Exploration noise mean. |
| `HorizonParameter` | `TunableParameter` | Planning horizon length (resizes internal buffers automatically). |
...
Ensure you have installed the optional autotuning dependencies:
```bash
pixi run -e autotuning test
```
Or if using `uv` or `pip`:
```bash
pip install cma evosax
```

### Static Analysis Type Errors
If you encounter `TypeError` when writing tests or sub-classing `TunableParameter` or `Optimizer`, you may need to bypass strict checking using `# type: ignore` or explicit casting:
```python
from typing import cast
import jax

# Using type ignores for abstract classes in tests
# obj = TunableParameter(...) # type: ignore

# Casting array types to satisfy basedpyright
# val = cast(jax.Array, my_array)
```
